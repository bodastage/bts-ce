...
def parse_and_import_zte_2g(parent_dag_name, child_dag_name, start_date, schedule_interval):
    ...
    t30 = BashOperator(
        ...
        bash_command='mv -f /mediation/data/cm/zte/parsed/bulkcm_gsm/* /mediation/data/cm/zte/parsed/backup 2>/dev/null || true',
        dag=dag)
    ...
    t31 = PythonOperator(
        task_id='clear_zte_2g_cm_tables',
        python_callable=clear_zte_2g_cm_tables,
        dag=dag)

    parse_zte_2g_cm_files = BashOperator(
      task_id='parse_zte_2g_cm_files',
      bash_command='java -jar /mediation/bin/boda_bulkcmparser.jar /mediation/data/cm/zte/raw/bulkcm_gsm /mediation/data/cm/zte/parsed/bulkcm_gsm /mediation/conf/cm/zte_cm_2g_blkcm_parser.cfg',
      dag=dag)

    import_zte_cm_data = BashOperator(
        task_id='import_zte_2g_cm_data',
        bash_command='python /mediation/bin/load_cm_data_into_db.py zte_bulkcm_gsm /mediation/data/cm/zte/parsed/bulkcm_gsm ',
        dag=dag)

    dag.set_dependency('check_if_zte_2g_raw_files_exist', 'backup_zte_2g_csv_files')
